...
@if(isset($nodeStatuses) && count($nodeStatuses) > 0)
<div class="row">
    <div class="col-xs-12" id="node-status-notifications">
        @foreach($nodeStatuses as $status)
            @if($status['status'] === 'online')
            <div class="alert alert-success alert-dismissable">
                <button type="button" class="close" data-dismiss="alert">&times;</button>
                <i class="fa fa-check-circle"></i> Node connected successfully.
            </div>
            @else
            <div class="alert alert-danger alert-dismissable">
                <button type="button" class="close" data-dismiss="alert">&times;</button>
                <i class="fa fa-exclamation-circle"></i> Node is offline. <a href="{{ route('admin.nodes') }}">View nodes</a>
            </div>
            @endif
        @endforeach
    </div>
